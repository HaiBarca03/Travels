...
const UserDetail = ({ userId, onClose }) => {
    ...
    return (
        <div className="user-detail">
            <button className="close-btn" onClick={onClose}>Đóng</button> {/* Nút đóng */}
            <h2>Chi tiết người dùng</h2>
            {dataUser && (
                <>
                    <p><strong>Tên:</strong> {dataUser.name}</p>
                    <p><strong>Email:</strong> {dataUser.email}</p>
                    <p><strong>Điện thoại:</strong> {dataUser.phone}</p>
                    <p><strong>Địa chỉ:</strong> {dataUser.addresses}</p>
                    <p><strong>Vai trò:</strong> {dataUser.role}</p>
                    <div>

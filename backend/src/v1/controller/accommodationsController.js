...
const createAccommodation = async (req, res) => {
    try {
        const { name, address, location, type, price, amenities, rating, reviews } = req.body;
...
const updateAccommodation = async (req, res) => {
    try {
        const { name, address, location, type, price, amenities, rating, reviews, deleteImages } = req.body;
        ...
        if (deleteImages && deleteImages.length > 0) {
            for (const public_id of deleteImages) {
                ...
                accommodation.images = accommodation.images.filter(img => img.public_id !== public_id);
            }
        }
...
const getAccommodationsByLocation = async (req, res) => {
    try {
        ...
        if (!accommodations || accommodations.length === 0) {
            return res.status(404).json({ success: false, message: 'No accommodations found for this location' });
        }
...
const getAllAccommodations = async (req, res) => {
    try {
        const accommodations = await Accommodations.find().populate('location');

        if (!accommodations || accommodations.length === 0) {
            return res.status(404).json({ success: false, message: 'No accommodations found' });
        }

...
const createTouristAttraction = async (req, res) => {
    try {
        const { name, description, location, activities, rating, price, reviews } = req.body;
...
const getTouristAttractionsByLocation = async (req, res) => {
    try {
        ...
        const attractions = await TouristAttraction.find({ location: locationId })
            .populate('location', 'country provinceCity')
...
const getRestaurantsByTouristLocation = async (req, res) => {
    try {
        const { id } = req.params;

        const attraction = await TouristAttraction.findById(id).populate('location');
        ...
        res.status(200).json({
            success: true,
            data: restaurants,
            message: 'Restaurants in the same location as the tourist attraction fetched successfully',
        });
    } catch (error) {
...
const getAccommodationByTouristLocation = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const accommodation = await Accommodation.find({ location: location._id });

        res.status(200).json({
            success: true,
            data: accommodation,
            message: 'Accommodation in the same location as the tourist attraction fetched successfully',
        });
    } catch (error) {
...
const updateTouristAttraction = async (req, res) => {
    try {
        const { name, description, location, activities, rating, price, deleteImages } = req.body;
        ...
        if (deleteImages && deleteImages.length > 0) {
            for (const public_id of deleteImages) {
                ...
                attraction.images = attraction.images.filter(img => img.public_id !== public_id);
            }
        }
